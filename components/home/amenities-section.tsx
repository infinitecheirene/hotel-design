...
export default function AmenitiesSection() {
  ...
  return (
    <section className="py-10 bg-linear-to-br from-red-50 to-red-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-6">
          <h2 className="text-3xl md:text-4xl font-bold mb-4 text-balance text-red-900">Premium Amenities</h2>
          <div className="w-24 h-1 mx-auto mb-6 bg-linear-to-r from-red-500 to-red-400" />
          <p className="text-lg max-w-2xl mx-auto text-pretty text-red-900">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {amenities.map((amenity, index) => (
            <div
              key={index}
              className="p-5 rounded-2xl transition-all duration-300 hover:scale-105 group bg-linear-to-br from-white to-red-50 shadow-lg border border-red-200"
            >
              <div className="text-center">
                <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 transition-all duration-300 group-hover:scale-110 bg-linear-to-br from-red-100 to-red-200 border-2 border-red-300">
                  <amenity.icon className="w-10 h-10 text-red-800" />
                </div>
                <h3 className="text-xl font-bold mb-3 text-red-800">{amenity.name}</h3>
                <p className="text-red-700">{amenity.description}</p>
              </div>
            </div>
          ))}
        </div>
