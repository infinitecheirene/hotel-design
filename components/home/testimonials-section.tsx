      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => (
            <div
              key={index}
              className="p-5 lg:p-8 rounded-2xl h-full transition-all duration-300 hover:scale-105 bg-linear-to-br from-white to-green-50 shadow-lg border border-red-200"
            >
              ...
              <h4 className="text-xl font-bold mb-3 text-red-800">{testimonial.name}</h4>
              <div className="flex items-center mb-4 text-red-700">
                <MapPin className="w-4 h-4 mr-2" />
                {testimonial.location}
              </div>
              <p className="text-sm italic leading-relaxed text-red-700">{testimonial.comment}</p>
            </div>
          ))}
        </div>
      </div>
